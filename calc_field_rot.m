...
%subfunction find_theta.m calculates the angle
%this angle is used in this function to find B'_x, B'_y
...
function s = calc_field_rot(bx, by, lat, lon, r, dn, step, overlap, limval, div, calc_sel)
...
for num = 1 : ostep : N
	...
	s.theta(arr_begin : arr_end, 1) = the;
	...
	for num2 = rng_beg : rng_end
		...
		if num2 >= arr_begin && num2 <= arr_end
	...
	s.limcheck(arr_begin : arr_end, 1) = bxdotsum / bydotsum;

	clearvars rng_beg rng_end arr_begin arr_end bxd_tmp byd_tmp runnum
